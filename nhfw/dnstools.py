...
def get_ns_info(fqdn):
    ...
    try:
        ...
        d = Domain.objects.get(name=domain)
    if not d.available:
        if d.last_update + timedelta(seconds=UNAVAILABLE_RETRY) > datetime.now():
...
class NamedServer:
    ...
    def __init__(self, nameserver, tsig):
    ...
    @property
    def _tsigkey(self):
        return dns.tsigkeyring.from_text({self.tsig['name']: self.tsig['key']})

    
    def query_record(self, strrec, rdtype):
        try:
            answer = self._resolver.query(strrec, rdtype)
    ...
    def update_record(self, zone, srecord, rdtype, svalue):
        current = None
        if rdtype in ["A", "AAAA"]:
            current = self.query_record(srecord + '.' + zone, rdtype)
        elif rdtype in ["PTR"]:
            current = self.query_record(srecord, rdtype)
        else:
            return None

        if current != None and svalue == current:
            nhfwlog.info("Nothing todo, skiping dns record update")
            return None

        upd = dns.update.Update(zone, keyring=self._tsigkey, keyalgorithm=dns.tsig.HMAC_SHA256)
        if current == None:
            upd.add(srecord, 60, rdtype, svalue)
        else:
            upd.replace(srecord, 60, rdtype, svalue)
        response = dns.query.tcp(upd, self.nameserver, timeout=UPDATE_TIMEOUT)
        rcode = response.rcode()
        #print(response, rcode)

        return rcode == dns.rcode.NOERROR
        

    def delete_record(self, zone, srecord, rdtype):
        current = None
        if rdtype in ["A", "AAAA"]:
            current = self.query_record(srecord + '.' + zone, rdtype)
        elif rdtype in ["PTR"]:
        ...
        if current == None:
            nhfwlog.info("Nothing todo, skip dns record deleting: the record not exists")
            return None

        upd = dns.update.Update(zone, keyring=self._tsigkey, keyalgorithm=dns.tsig.HMAC_SHA256)
        upd.delete(srecord, rdtype)
        response = dns.query.tcp(upd, self.nameserver, timeout=UPDATE_TIMEOUT)
        rcode = response.rcode()
        #print(response, rcode)
